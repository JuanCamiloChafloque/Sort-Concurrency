...
//Método principal que llamaran los hijos para realizar el ordenamiento del archivo
void mainQuicksort(struct Archivo *arc){
    quicksort(arc, 0, arc->numProcesos - 1);
    escribirArchivo(arc, arc->nombre, 0);
}

//Método principal que llamaran los hijos para realizar el ordenamiento del archivo
void mainQuicksortReversed(struct Archivo *arc){
    quicksortReversed(arc, 0, arc->numProcesos - 1);
    escribirArchivo(arc, arc->nombre, 0);
}
...
int main(int argc, char* argv[]){
    ...
    for(int i = 0; i < numHilos; i++){
        if(rev == 0){
            pthread_create(&hilos, NULL, (void*)mainQuicksort, &archivosHijos[i]);
        }else{
            pthread_create(&hilos, NULL, (void*)mainQuicksortReversed, &archivosHijos[i]);
        }
    }
